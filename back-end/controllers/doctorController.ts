  ...
  try {
    ...
    if (req.user?.role !== "doctor") {
      return res
        .status(403)
        .json({
          message: "Only users with 'doctor' role can create a doctor profile.",
        });
    }
  ...
  try {
    ...
    if (!doctorProfile || doctorProfile.status !== "approved") {
      return res
        .status(403)
        .json({
          message: "Doctor profile not approved or missing/unapproved.",
        });
    }

    const workingHours: IWorkingHour[] = doctorProfile.workingHours;
    const slotDuration: number = doctorProfile.slotDuration;

    if (!workingHours || workingHours.length === 0) {
      return res
        .status(400)
        .json({ message: "Doctor has not set working hours." });
    }
    ...
    while (currentDate <= end) {
      ...
      if (dailySchedule) {
        const [startHour, startMinute] = dailySchedule.startTime
          .split(":")
          .map(Number);
        const [endHour, endMinute] = dailySchedule.endTime
          .split(":")
          .map(Number);
