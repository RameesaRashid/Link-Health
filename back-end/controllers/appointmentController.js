...
//     })
// }
export const searchAvailableSlots = async (req, res) => {
    try {
        ...
        if (doctorId) {
            ...
            const doctors = await Doctor.find({ specialty: specialty }).select('_id');
...
export const bookAppointment = async (req, res) => {
    try {
        ...
        const updatedSlot = await Slot.findOneAndUpdate({ _id: slotObjectId, isBooked: false }, { $set: { isBooked: true, patientId: patientObjectId } }, { new: true });
        if (!updatedSlot) {
...
export const viewPatientAppointments = async (req, res) => {
    try {
        const patientId = req.user?.userId;
        if (!patientId || req.user?.role !== 'patient') {
